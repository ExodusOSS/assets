...
const baseAsset = assetPlugin.createAsset({ assetClientInterface: dummyAssetClientInterface })
...
describe('bicointestnet addresses', () => {
  it('is valid segway address ', () => {
    expect(baseAsset.address.validate('bc1qgxz38crda20mlre427ygezgu67kqvqfr4heayn')).toBe(false) // mainnet
    expect(baseAsset.address.validate('tb1qh3v7p0gu6s5xeucy9qrq92khl8av3agxru8x0y')).toBe(true) // testnet
  ...
  it('is valid legacy address', () => {
    expect(baseAsset.address.validate('1AELPVvN9aDdWsR7hCkpVgA2WAuDjuPDB3')).toBe(false) // mainnet
    expect(baseAsset.address.validate('mpkHgZ1LxbetHytjQmjCKbNMNAVviZQ4UA')).toBe(true) // testnet and regtest
  })
})
