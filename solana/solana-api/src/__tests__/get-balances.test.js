...
describe('get-balances', () => {
  ...
  test('has balance with non empty account', () => {
    // Arrange:
    const accountState = new SolanaAccountState({
  ...
  test('has balance with empty account', () => {
    // Arrange:
    const accountState = new SolanaAccountState({
      balance: solana.currency.baseUnit(100),
      rentExemptAmount: solana.currency.baseUnit(14),
    ...
    const zero = solana.currency.ZERO
    expect(balances.balance).toEqual(solana.currency.baseUnit(100))
    expect(balances.spendableBalance).toEqual(solana.currency.baseUnit(100))
    expect(balances.total).toEqual(solana.currency.baseUnit(100))
    expect(balances.spendable).toEqual(solana.currency.baseUnit(100))
    expect(balances.networkReserve).toEqual(zero)
    expect(balances.walletReserve).toEqual(zero)
  })
  ...
  test('balance is equal to (spendableBalance + withdrawable + locked + pending + accountReserve)', () => {
    ...
    const expectedSpendableBalance = balance
      ...
      .sub(accountState.rentExemptAmount)
      .clampLowerZero()
  ...
  test('balance is fixed when unconfirmed txs', () => {
    ...
    const expectedSpendableBalance = solana.currency.baseUnit(96_999_970_000)
    ...
    expect(balances.walletReserve.toDefaultString()).toEqual(zero.toDefaultString())
  })
})

describe('get-balances allowSendingAll', () => {
  const getBalances = getBalancesFactory({ stakingFeatureAvailable: true, allowSendingAll: true })
  solana.accountReserve = solana.currency.baseUnit(20)
  ...
  test('has balance', () => {
    ...
    expect(balances.total).toEqual(solana.currency.baseUnit(100))
    expect(balances.spendable).toEqual(solana.currency.baseUnit(100))
    expect(balances.networkReserve).toEqual(solana.currency.ZERO)
    expect(balances.walletReserve).toEqual(zero)
  })
