...
const currency = UnitType.create({
  satoshis: 0,
  bits: 2,
  BTC: 8,
})
...
describe('create', () => {
  it('works', () => {
    const a = new FeeData({ config: { fee: '1 BTC' }, mainKey, currency })
    expect(a.fee).toEqual(currency.BTC(1))
  })

  it('Not convert to default unit', () => {
    const a = new FeeData({ config: { fee: '10000 satoshis' }, mainKey, currency })
...
describe('update', () => {
  ...
  test('empty mainkey', () => {
    const a = new FeeData({ config: { fee: '1 BTC' }, currency })
